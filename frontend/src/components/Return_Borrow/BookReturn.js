...
const ReturnBook = () => {
  ...
  const handleReturn = async () => {
    if (confirmReturn === "Return" && selectedBook) {
      ...
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_URI}/books/Return`,
          {
            method: "PUT",
            headers: {
        ...
        console.error("Error Returng in book:", error);
      }
    } else {
      alert(
        'Please type "Return" in the confirmation box to check in the book.'
      );
    }
  };
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold text-center mb-6">Book Return</h1>
      {!selectedBook ? (
        <>
          <SearchBar
