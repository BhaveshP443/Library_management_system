...
const BorrowReceipt = lazy(() => import('./BorrowReceipt'));
...
const BookBorrow = () => {
  ...
  const handleBorrow = () => {
    if (confirmBorrow === 'Borrow' && searchedBook && searchedBorrower) {
      const bookId = searchedBook._id;
      const borrowerId = searchedBorrower._id;

      fetch(`${process.env.REACT_APP_API_URI}/books/Borrow`, {
        method: 'PUT',
        headers: {
        ...
        .catch((error) => {
          console.error('Error Returng out book:', error);
          alert('Error Returng out book. Please try again.');
        });
    } else {
      alert('Please type "Borrow" in the confirmation box and select both a book and a borrower.');
    }
  };

  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold text-center mb-6">Book Borrow</h1>
      {!showSelectedItems && (
          <SearchBar
      ...
      {showSelectedItems && (
        <>
        <BorrowReceipt selectedBook={selectedBook} selectedBorrower={selectedBorrower} />
