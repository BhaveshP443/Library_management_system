...
const BorrowReceipt = ({ selectedBook, selectedBorrower }) => {
  const handleSaveAsPDF = () => {
    ...
    const textWidth = pdfDoc.getStringUnitWidth('Borrow Book Receipt') * 6;

    const centerX = (width - textWidth) / 2;

    pdfDoc.text('Borrow Book Receipt', centerX+3, 10);
    ...
    pdfDoc.save(`BorrowReceipt-${selectedBook.title}.pdf`);
  };
  
    return (
      <div className="bg-gray-700 text-left rounded-lg p-8 w-screen max-w-md mb-4">
        <div className=' pl-2'>
          <h1 className="text-2xl font-bold text-center mb-4">-:Borrow Receipt:-</h1>
